...
export default function SubcategoryPage() {
  ...
  const subIdStr = params.subId as string
  const subcategoryIdNum = Number(subIdStr)
    ...
    const loadMeta = async () => {
      try {
        ...
        if (res.ok && json?.success) {
          ...
          const found = cats.find((c: any) => Number(c.id) === categoryIdNum)
          setCategoryName(found?.name || '')
          const subs: any[] = found?.subcategories || []
          setSubcategories(subs.map((s: any) => ({ id: Number(s.id), name: String(s.name || '') })))
          const subFound = subs.find((s: any) => Number(s.id) === subcategoryIdNum)
  ...
  const loadMoreResources = async (
    ...
    try {
      ...
      const url = `/api/resources?page=${requestedPage}&size=${size}&subcategoryId=${subcategoryIdNum}&sort=${activeSort}`
  ...
  // Reset and initial load when subcategory changes
  useEffect(() => {

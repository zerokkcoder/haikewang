...
export default function AdminCategoriesPage() {
  ...
  const addCategory = async () => {
    if (!name.trim()) return
    const res = await fetch('/api/admin/categories', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, sort: sort === '' ? 0 : Number(sort) })
    })
    const data = await res.json()
    if (data.success) {
      setName('')
  ...
  const saveEdit = async () => {
    ...
    const res = await fetch(`/api/admin/categories/${editingId}`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: editName, sort: editSort === '' ? undefined : Number(editSort) })
    })
    const data = await res.json()
    if (data.success) {
      setEditingId(null)
  ...
  const addSub = async (catId: number) => {
    ...
    const res = await fetch('/api/admin/subcategories', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ categoryId: catId, name: subName, sort: subSort === '' ? 0 : Number(subSort) })
    })
    const data = await res.json()
    if (data.success) {
      setSubName('')
  ...
  const saveSubEdit = async () => {
    ...
    const res = await fetch(`/api/admin/subcategories/${subEditId}`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: subEditName, sort: subEditSort === '' ? undefined : Number(subEditSort) })
    })
    const data = await res.json()
    if (data.success) {
      setSubEditId(null)
                  ...
                  <tr className="border-t">
                    ...
                    <td className="py-2 pr-4">
                      {editingId === c.id ? (
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => { setEditingId(c.id); setEditName(c.name); setEditSort((c as any).sort ?? 0) }} className="btn btn-secondary btn-sm">编辑</button>
                              ...
                              {subList.map(sc => (
                                <div key={sc.id} className="flex items-center justify-between p-2 rounded bg-card shadow-sm">
                                  <div className="flex-1">
                                    {subEditId === sc.id ? (
                                      <input value={subEditName} onChange={(e) => setSubEditName(e.target.value)} className="input flex-1" />
                                    ) : (
                                  ...
                                  <div className="flex items-center gap-2">
                                    {subEditId === sc.id ? (
                                      ...
                                      <>
                                        <button onClick={() => { setSubEditId(sc.id); setSubEditName(sc.name); setSubEditSort((sc as any).sort ?? 0) }} className="btn btn-secondary btn-sm">编辑</button>
                                        <button

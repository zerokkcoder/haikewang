...
export default function VIPFloatingButton({ className = '' }: { className?: string }) {
  return (
    <Link
      href="/vip"
      className={`relative w-12 h-12 rounded-full bg-linear-to-r from-yellow-400 to-orange-500 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 group flex items-center justify-center ${className}`}
      aria-label="升级VIP会员"
    >
